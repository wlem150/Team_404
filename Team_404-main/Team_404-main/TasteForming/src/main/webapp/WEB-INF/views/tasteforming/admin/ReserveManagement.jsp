...
<body class="hold-transition sidebar-mini">
		...
		<div class="content-wrapper">
			...
			<section class="content-header">
				<div class="container-fluid">
					<div class="row mb-2">
						<div class="col-sm-6">
							<h1 style="font-size:3rem; font-weight:bold;">예약관리</h1>
						</div>
						<div class="col-sm-6">
				...
				<div class="card">
					...
					<div class="card-body">
						<div id="jsGrid1" class="jsgrid"
									...
									<tbody>
										...
										<c:forEach items="${reserve }" var="reserve" varStatus="loop">
											...
											<tr class="jsgrid-edit-row" id="edit-row-${loop.index}"
												style="display: none;">
												<td class="jsgrid-cell" style="width: 20px;"><input
													type="text" value="${reserve.order_No }" readonly></td>
												<td class="jsgrid-cell" style="width: 50px;"><input
													type="text" value="${reserve.userId }" readonly></td>

												<td class="jsgrid-cell" style="width: 50px;"><input
													type="text" value="${reserve.res_Name }" readonly></td>

												<td class="jsgrid-cell" style="width: 50px;"><input
													type="date" value="${reserve.order_Date }" readonly></td>
												<td class="jsgrid-cell" style="width: 20px;"><input
													type="text" value="${reserve.head_Cnt }" readonly></td>
												<td class="jsgrid-cell" style="width: 20px;"><input
													type="text" value="${reserve.hour }" readonly></td>
												<td class="jsgrid-cell" style="width: 50px;"><input
													type="date" value="${reserve.cancel_Date }" readonly></td>
												<td
											...
											<tr
												...
												<td class="jsgrid-cell jsgrid-align-center"
													style="width: 20px;"><c:out
														value="${reserve.order_No }" /></td>
												<td class="jsgrid-cell jsgrid-align-center"
													style="width: 50px;"><c:out value="${reserve.userId}" /></td>
												<td class="jsgrid-cell jsgrid-align-center"
													style="width: 50px;"><c:out
														value="${reserve.res_Name}" /></td>
												<td class="jsgrid-cell jsgrid-align-center"
													style="width: 50px;"><fmt:formatDate
														value="${reserve.order_Date}" pattern="yyyy-MM-dd" /></td>


												<td class="jsgrid-cell jsgrid-align-center"
													style="width: 20px;"><c:out
														value="${reserve.head_Cnt}" /></td>
												<td class="jsgrid-cell jsgrid-align-center"
													style="width: 20px;"><c:out value="${reserve.hour}" /></td>
												<td class="jsgrid-cell jsgrid-align-center"
													style="width: 50px;"><fmt:formatDate
														value="${reserve.cancel_Date}" pattern="yyyy-MM-dd" /></td>
												<td
													class="jsgrid-cell jsgrid-control-field jsgrid-align-center"
													style="width: 50px;"><input
							...
							<form action="/tasteforming/admin/ReserveManagement"
	...
	<script>
		...
		$(document).on("click", ".jsgrid-update-button", function() {
		    ...
		    var order_No = row.find("td:eq(0) input").val();
		    var head_Cnt = row.find("td:eq(4) input").val();
		    var hour = row.find("td:eq(5) input").val();
		    var cancel_Date = row.find("td:eq(6) input").val();

		    
		    var dateArray = cancel_Date.split("-");
		    var convertedDate = dateArray[0] + "/" + dateArray[1] + "/" + dateArray[2];
		    // form 태그 생성
		    var form = $("<form>").attr({
		...
		$(document).on("click", ".jsgrid-delete-button", function() {
			var order_No = $("#data-row-${loop.index} td:first-child").text();
		...
		$(document).on("click", ".jsgrid-delete-button", function () {
			var order_No = this.closest('tr').querySelector('td:first-child').innerText;
